...
def load_duration_curve(t,dic,y,legend,decison_var,path2output,cmap):
    ...
    areas = [p.patch(xy[0],xy[1],color=colors[i], alpha=0.8, line_color=None,muted_alpha=0.2) for i,xy in enumerate(zip(x,y)) ]
    items = [("sum",   [line] ),("Load Duration Curve", [line_heat_demand])]
...
def cmapping(tec,path2cmap=-1):
    cmap = {j:cc.b_glasbey_category10[i] for i,j in enumerate(tec)} 
    if path2cmap == -1:
        return cmap
...
def plot_solutions(show_plot=False, path2json=path2json, 
    ...
    try:
        ...
        cmap = cmapping(dic["Technologies:"],path2cmap) if cmap == -1 else cmap
        ...
        costs = ["Total Coldstart Costs","Total Ramping Costs","Total Operational Costs","Total Fuel Costs","Total Electricity cost","Total CO2 Costs","Total Investment Costs (of new build plants and heat storages)","Total Investment Costs of (of existing power plants and heat storages)",
                 "PV Investment Cost","Refurbishment Cost"]
        singel_values = ["Total Costs","Total Costs (including Electricity cost)","Total Revenue From Electricity","Total Thermal Generation","Total Final Energy","Total LCOH","Total CO2 Emissions","Total Heat Demand","Peak Load","CO2 Price",
        ...
        costs_hg = ["Anual Investment Cost:",
                    ...
                    "Coldstart Costs:",
                    "Revenue From Electricity:"]
        values_hg = ["Thermal Generation Mix:","Final Energy Mix","Installed Capacities:","Total CO2 Emission:","emission factor:","max thermal efficiency","Full Load Hours:","LCOH:","Turn Over Rate:"]
        heat_generator_topics= costs_hg+values_hg
        ...
        p9 = plot_table(data_topics,dic,path2output)
        ...
        p8 = costBarStack_chart(dic,costs_hg,path2output,cmap)
        decision_vars += ["Total CO2 Emission:","Full Load Hours:","LCOH:","Turn Over Rate:"] 
        p11 = plotExtra_table(heat_generator_topics,dic,path2output)
        l2 = layout([[p8],[p11]])
