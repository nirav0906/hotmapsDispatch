...
from CM.CM_TUWdispatch.scenario_chart import scenario_chart,create_chart_definitions,prerpare_data
...
#%% when starting server delete folders that are older then <x> hours old
# hours = 2
# for to_delete in Path(path_static).glob("*"):
#     try:
#         if to_delete.is_dir() and datetime.datetime.strptime(to_delete.name,"%Y-%m-%d-%H-%M-%S-%f") < datetime.datetime.now()- datetime.timedelta(hours=hours):
...
def customCmap(list_of_tuples):
    ...
    for liste,palette in list_of_tuples:
        l = len(liste)
        palett_length = max(palette)
        if l > palett_length:
#            duplicate colors of palette, to avoid out of range
            map_index = dict(zip(range(l),list(range(palett_length))*(l-palett_length)))
        for i,j in enumerate(liste):
            if l<3:
                cmap[j]=palette[3][i]
            elif l>palett_length:
                cmap[j] = palette[palett_length][map_index[i]]
            else:
                try:
                    cmap[j]=palette[l][i]
                except:
                    cmap[j]=palette[palett_length][i]
                    
    return cmap
# =============================================================================
from bokeh.palettes import Blues,Greens,Greys,Oranges,Purples,Reds,Pastel1,BrBG,Category20c,Category20b,PiYG,YlOrBr,RdPu,Magma,Pastel2
...
input_list_gui= ['name',
 ...
 'OPEX fix (EUR/MWa)',
 'OPEX var (EUR/MWh)',
 'Coldstart Costs (EUR/Start)',
 'life time',
 'renewable factor',
 'must run [0-1]',
  "Maximal Available Potential [MWh]",
  "Maximal Output Power [MW]",
  "Excess Heat Source Price [EUR/MWh]",
  "Ramping Costs [EUR/MWh]"]

input_list = input_list_gui + ["type", 'COP source temperature sensitivity (1/°C)',
 'COP flow temperature sensitivity (1/°C)',
 'COP return temperature sensitivity (1/°C)',
 'min shutoff source temperature (°C)',
 'fade out temp where power reduction starts (°C)',
 'Source Temperature (°C)',
 'Nominal Inlet Temperature (°C)',
 'Nominal Return Temperature (°C)',
 'Refrigerants',
 'Maximum Power of the Heatpump Compressor Pel (MW)',
 'Minimum Output Power Factor [0-1]',
]
...
widgets_keys = ["Radiation","Temperature","Electricity price",
                ...
                "Inlet Temperature","Return Temperature","River Temperature", "Waste Water Temperature"]
# Specify model names for the external data
model_keys = ["radiation","temp","electricity","sale_electricity","demand_th",
              "inlet_temp","return_temp","river_temp","wastewater_temp"]
...
for sheet in pd.ExcelFile(path_parameter).sheet_names:
    if sheet in sheets:
        continue
    _name = str(pd.read_excel(path_parameter,sheet).columns[0]).strip() # heat pump type + refrigerant
    _hp_type =str(pd.read_excel(path_parameter,sheet).columns[1]).strip() #type
    _source_temp = str(pd.read_excel(path_parameter,sheet).columns[3]).strip().replace(" ","") #source temp
    
    if _hp_type not in hp_types:
        hp_types.append(_hp_type)
        heat_pumps[_hp_type] = dict()
        flow_temp_dic[_hp_type] = dict()
        return_temp_dic[_hp_type] = dict()
        hp_structure[_hp_type] = dict()
        
    if _name not in list(hp_structure[_hp_type]):
        hp_structure[_hp_type][_name] = []
        heat_pumps[_hp_type][_name] = dict()
        flow_temp_dic[_hp_type][_name]= dict()
        return_temp_dic[_hp_type][_name]= dict()
    
    hp_structure[_hp_type][_name].append(_source_temp)

    cop_matrix = pd.read_excel(path_parameter,sheet,skiprows=[0]).fillna(0).set_index("COP")
    heat_pumps[_hp_type][_name][_source_temp] = cop_matrix
    flow_temp_dic[_hp_type][_name][_source_temp] = cop_matrix.columns.values.tolist()
    return_temp_dic[_hp_type][_name][_source_temp] = cop_matrix.index.values.tolist()
...
select_tec2_options= {typ:data_tec[data_tec["type"] == typ]["name"].values.tolist() for typ in list(data_tec["type"])}
select_tec2_options["CHP Steam Extraction"] = select_tec2_options.pop("CHP-SE")
select_tec2_options["CHP Back Pressure"] = select_tec2_options.pop("CHP-BP")
...
#%% Main Functionality:  Widgets, callbacks, layouts etc.
    ...
    widgets_data = [ dict(data=radiation,
                    ...
                    dict(data=load_profiles,
                          ...
                          labels=lab,
                          unit="MW"),
                    dict(data= inlet_temperature,
                          dic= inlet_temperature_name_map,
                          dic_inv = invert_dict(inlet_temperature_name_map),
                          labels=lab, 
                          unit="°C"),
                    ...
                    dict(data = river_temperature,
                          ...
                          labels=lab, 
                          unit="°C"),
                    dict(data = wastewater_temperature,
                          dic = wastewater_temperature_name_map,
...
# =============================================================================
    ...
    def check_scenarios(scenario_mapper):
        ...
        no_cap = [(sc,sub_sc)  for sc in list(scenario_mapper) for sub_sc in scenario_mapper[sc] if (max(profiles_table[_hd][sc][sub_sc].values()) - sum(heat_generator_table[sc][sub_sc][input_list[1]]) - sum(heat_storage_table[sc][sub_sc]["maximum unloading power  [MW]"])) > 0]
        if no_hg != []:
            ...
            if invest_button.active:
                return True
            sc,sub_sc = zip(*no_cap)
            _x=[f"Scenario:{sc} with Sub Scenario:{sub_sc}" for sc,sub_sc in no_cap]
            return f"""The installed capacities are not enough to cover the load for folowing {_x}"""
...
#            _nth.pop(k,None)
    ...
    def get_folder_structure(path,downlad_flag=False):
        ...
        elif file.split(".")[0] in ["output_scenario_charts","Data (hourly values)","Data (single values)","Generator Data (hourly values)","Generator Data (single values)"]:
            return file
        else:
    ...
    def save_scenario(path_scenarios_root,sc,sub_sc,output_data,cmap=-1):
        ...
        if output_tabs == "Error4":
            ...
            output_file(path_download_html,title=f"Dispatch output: {sc} # {sub_sc}",mode="inline")
            save(output_sc)
            ...
            json2xlsx(output_data,path_download_xlsx,tidy=True) #XXX: change function with return values
            # -- input XLSX
...
# =============================================================================   
    def compare_plot(dict_of_solutions,path2output):
        ...
        sources1,len_sub_sc1 = prerpare_data(out_dict["Data (single values)"])       
        sources2,len_sub_sc2 = prerpare_data(out_dict["Generator Data (single values)"])
        ...
        return output_html 
# =============================================================================
    def create_cmap_for_all_scenarios(strip=False):
        hgs = dict()
        for sc in list(scenario_mapper):
            for sub_sc in scenario_mapper[sc]:
        ...
        if strip:
            _cmap_striped = customCmap( [ ([x.strip() for x in liste],cmap_glob[typ]) for typ,liste in hgs.items()] )
            _hg_hs = [x for typ,liste in hgs.items() for x in liste]
            return {_x:_cmap_striped[_x.strip()] for _x in _hg_hs}
        else:
            return customCmap( [ (liste,cmap_glob[typ]) for typ,liste in hgs.items()] )

    def create_cmap_for_all_scenarios2():
        hgs = dict()
        for sc in list(scenario_mapper):
            for sub_sc in scenario_mapper[sc]:
                _hgs = { typ: df["name"].values.tolist() for typ,df in heat_generator_table[sc][sub_sc].groupby("type") } 
        ...
        _hg_hs = [x for typ,liste in hgs.items() for x in liste]
        _hg_hs_striped = list(set([x.strip() for x in _hg_hs]))
        
        _colorset = colorcet.b_glasbey_hv[:len(_hg_hs_striped)]
        _cmap_striped = dict(zip(_hg_hs_striped,_colorset))
...
# =============================================================================            
            if scenario_button.active:
                ...
                j=0
                ok,message = True,""
                cmap = create_cmap_for_all_scenarios2()
                # pickle.dump(cmap,open(r"C:\Users\hasani\Desktop\cmap2.dat","wb"))
                # sys.exit()
                for sc in list(scenario_mapper):
...
# =============================================================================
    ...
    #                selection1 = data_table_heat_storage.source.selected.indices #XXX:  > 0.12.10
                ...
                if solutions == "Error1":
                    ...
                    div_spinner.text = notify(f"Error: The installed capacities are not enough to cover the load !!!\n{_message}","red") 
                elif solutions == "Error3#":
...
#                        output.tabs = output_tabs #TODO: render only when a toggle button is active
                        ...
                        save(Tabs(tabs=output_tabs)) #XXX: not working , dont know why, need to create new objecs !!
...
#                        time_id = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f")
                        ...
                        json2xlsx(solutions,path_download_xlsx,tidy=True)
...
#                        time_id = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f")
                        ...
                        path_download_xlsx = list(Path(path_download_xlsx).parent.glob("*.xlsx"))
...
# =============================================================================
    def update_pmax(attrname, old, new):
        ...
        _data2= pd.DataFrame(data_table_heat_storage.source.data)[ "maximum unloading power  [MW]"].apply(pd.to_numeric, errors='ignore')
...
# =============================================================================
    ...
    hp_compressor_power_label = TextInput(placeholder="max. Compressor Power", title="Maximum Power of the Heatpump Compressor Pel (MW)")
   
    cop_source_sens_label = TextInput(placeholder="COP s. temp. sens", title="COP source temperature sensitivity (1/°C)")
    cop_inlet_sens_label = TextInput(placeholder="COP f. temp. sens", title="COP flow temperature sensitivity (1/°C)")
    cop_return_sens_label = TextInput(placeholder="COP r. temp. sens", title="COP return temperature sensitivity (1/°C)")
    source_temp_min_label = TextInput(placeholder="min. source temp", title="min shutoff source temperature (°C)")
    source_temp_fadeout_label = TextInput(placeholder="source fadeout temp", title="fade out temp where power reduction starts (°C)")
    flow_temp = Select(title="Inlet Temperature")
    return_temp = Select(title="Return Temperature")
    source_temp = Select(title="Source Tempearture",value="")
    refrigerant = Select(title="Refrigerant",value="")
    ...
    c_coldstart_label = TextInput(placeholder="-", title="Coldstart Costs (EUR/Start)")
    ...
    output_power_min_factor_label = Slider(start=0, end=1, value = 0, step = .01 , title="Factor of min. possible Output Power(%)")
    name_label = TextInput(placeholder="-", title="Name",disabled=False)
    potential_label = TextInput(placeholder="-", title="Maximal Available Potential [MWh]",disabled=False)
    power_label = TextInput(placeholder="-", title="Maximal Output Power [MW]",disabled=False)    
    excess_heat_source_price = TextInput(placeholder="-", title="Excess Heat Source Price [EUR/MWh]",disabled=False)
    ...
    widgets_list= [
            ...
            ik_label, opex_fix_label, opex_var_label,c_coldstart_label,lt_label,
            renewable_factor, must_run_box,potential_label,power_label,excess_heat_source_price,c_ramp_label,select_tec,cop_source_sens_label,
            cop_inlet_sens_label,cop_return_sens_label,source_temp_min_label,
            source_temp_fadeout_label,source_temp,flow_temp,return_temp,refrigerant,
            hp_compressor_power_label, output_power_min_factor_label,
            ]

    widgets_dict = dict(zip(input_list,widgets_list))
    cop_widget_list = [return_temp, flow_temp, cop_label, cop_source_sens_label, 
                       cop_inlet_sens_label, cop_return_sens_label, 
                       hp_compressor_power_label,  source_temp_min_label,
                       source_temp_fadeout_label,source_temp,refrigerant]
    hg_widget_list = [select_tec,select_tec2, installed_cap, output_power_min_factor_label, n_th_label, n_el_label,
            ik_label, opex_fix_label, opex_var_label,c_coldstart_label,lt_label,
...
# =============================================================================
    cop_layout= column([row([refrigerant,source_temp]),row([return_temp,flow_temp]),cop_label,row([hp_compressor_power_label]),cop_source_sens_label, cop_inlet_sens_label, cop_return_sens_label,
                        source_temp_min_label,source_temp_fadeout_label])
    ...
    finance_param_layout = column(children=[row(children=[ik_label,lt_label]),
                                            row(children=[opex_fix_label,
                                                          opex_var_label]),
                                            row(children=[c_coldstart_label])])
    model_param_layout = widgetbox(must_run_box,renewable_factor,installed_cap, output_power_min_factor_label,potential_label,power_label,excess_heat_source_price,c_ramp_label)
    ...
    def add_to_cds():
        ...
        for x in input_list:
            if x == "name":
                ...
                new_data["type"] = select_tec_mapper[widgets_dict[x].value]
            elif x in input_list[-9:-1] and select_tec.value not in ["heat pump","Heat Pump Air", "Heat Pump River Water", "Heat Pump Wastewater"]: 
                new_data[x] = "" 
            else:
                new_data[x] = str(widgets_dict[x].value).strip().replace(" ","").replace("°C","")
...
# =============================================================================
    def add_heat_pump():
        ...
        temp,in_temp,re_temp= [f"""{int(data_tec[data_tec["name"] == select_tec2.value][x].values[0])}°C""" for x in ['Source Temperature (°C)','Nominal Inlet Temperature (°C)','Nominal Return Temperature (°C)']]
        ref = str(data_tec[data_tec["name"] == select_tec2.value]["Refrigerants"].values[0])
        typ = select_tec.value
        
        refrigerant.options = list(heat_pumps[typ]) # Caution: this calls the cop_callback, thus need of try block
        refrigerant.value = ref
        
        source_temp.options = list(flow_temp_dic[typ][ref])
        source_temp.value = temp
        
        flow_temp.options = flow_temp_dic[typ][ref][temp]
        # Caution: this calls the cop_callback, thus need of try block  
        flow_temp.value = in_temp if in_temp in flow_temp.options else flow_temp.options[0] 
        
        return_temp.options = return_temp_dic[typ][ref][temp]
        # Caution: this calls the cop_callback,
        return_temp.value = re_temp if re_temp in return_temp.options else return_temp.options[0]  
        
        cop_label.value = str(heat_pumps[typ][ref][temp].loc[return_temp.value,flow_temp.value])
...
# =============================================================================
    def add_tec(name,old,new):
        ...
        if select_tec.value in ["heat pump","Heat Pump Air", 
                                "Heat Pump River Water", 
                                "Heat Pump Wastewater",
                                "CHP Back Pressure", "CHP Steam Extraction", 
                                "boiler"]:
        ...
        elif select_tec.value == "Heat Pump Ind. Wasteheat":
            select_tec2.disabled = True
            n_th_label.disabled = True
            cop_label.disabled = False
            cop_inlet_sens_label.disabled = False
            cop_return_sens_label.disabled = False
            cop_label.value = n_th_label.value
        else:
            select_tec2.disabled = True
...
# =============================================================================
    def auto_load(name,old,new):
        ...
        if select_tec.value in ["heat pump","Heat Pump Air", "Heat Pump River Water", "Heat Pump Wastewater"]:
            add_heat_pump()
        else:
        ...
        c_coldstart_label.value = str(data_tec[data_tec["name"] == select_tec2.value]["Coldstart Costs (EUR/Start)"].values[0])
...
# =============================================================================
    def cop_callback(name,old,new):
        try: # because of empty fields at the start no COP can be found
            typ = select_tec.value
            ref = refrigerant.value
            if ref not in list(heat_pumps[typ]):
                refrigerant.options = list(heat_pumps[typ])
                refrigerant.value = refrigerant.options[0]
                
            temp = source_temp.value
            if temp not in list(heat_pumps[typ][ref]):
                source_temp.options = heat_pumps[typ][ref]
                source_temp.value = source_temp.options[0]
            
            if flow_temp.value not in flow_temp_dic[typ][ref][temp]:
                flow_temp.options = flow_temp_dic[typ][ref][temp]
                flow_temp.value = flow_temp.options[0] 

            if return_temp.value not in return_temp_dic[typ][ref][temp]:      
                return_temp.options = return_temp_dic[typ][ref][temp]
                return_temp.value = return_temp.options[0] 
            ...
            pass
# =============================================================================
    def cop_update_callback(name,old,new):
        n_th_label.value = cop_label.value
        #if the heatpumps cop changes also the installed cap has to be recalculated
        try:
            installed_cap.value = float(n_th_label.value) * float(hp_compressor_power_label.value)
        except:
            installed_cap.value = 0
# =============================================================================        
    def hp_compressor_power_update_callback(name,old,new):
        #if compressor power changes also the installed cap has to be recalculated
        try:
            installed_cap.value = float(n_th_label.value) * float(hp_compressor_power_label.value)
        except:
            installed_cap.value = 0
# =============================================================================        
    def cap_update_callback(name,old,new):
        #if compressor power changes also the installed cap has to be recalculated
        try:
            hp_compressor_power_label.value = str(round(float(installed_cap.value)/float(n_th_label.value),2))
        except:
            hp_compressor_power_label.value = str(0)
...
# =============================================================================
    ...
    source_temp.on_change('value', cop_callback)
    refrigerant.on_change('value', cop_callback)
    energy_carrier_select.on_change('value', change_name_if_carrier_changes)
    cop_label.on_change('value',cop_update_callback) 
    hp_compressor_power_label.on_change('value',hp_compressor_power_update_callback)
    installed_cap.on_change('value',cap_update_callback)
...
#            sizing_mode='scale_width')  #XXX: scale_width -> invest_button not clickable
    ...
    return doc
#%% Main Function
def main(solver,openbrowser,port,num_procs=0):
    ...
    if port == -1:   
        try:
            ...
            print(-1)
            host="localhost"
            port= "".join([str(random.randint(1,10)) for _ in range(4)])
    else:
        host="localhost"

    config = dict(allow_websocket_origin=[f"{host}:{port}"],port=int(port))        
    bokeh_app = Application(FunctionHandler(modify_doc))
    bokeh_app_2 = Application(DirectoryHandler(filename = path2data)) #FIXME: for downloading files
    base_config = {'/': bokeh_app, '/download':bokeh_app_2}
...
# =============================================================================
    try:
        ...
        config1 = {**config,**dict(num_procs=num_procs)}
        server = Server(base_config, **config1)
        print("Start Multi Processess",end="")
# =============================================================================
#     
# =============================================================================
        ...
        config2 = {**config,**dict(io_loop=io_loop)}

        server = Server(base_config, **config2)
# =============================================================================
# 
# =============================================================================
    url = f"http://{host}:{port}"
    print(f"Open HotmapsDispath Application on {url}")
